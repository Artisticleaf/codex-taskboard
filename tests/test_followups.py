...
class FollowupTests(unittest.TestCase):
    ...
    def test_build_standard_followup_prompt_includes_footer_contract(self) -> None:
        ...
        self.assertIn("Taskboard 操作简介：", prompt)
    ...
    def test_process_followups_nudge_mentions_bound_proposal(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("当前绑定：", delivered_prompt)
            self.assertIn("本轮可靠结果、关键诊断结论和 next bounded action 默认写回当前 proposal", delivered_prompt)
            self.assertIn("写回 proposal/history 时必须说人话", delivered_prompt)
    ...
    def test_build_continuous_research_prompt_mentions_closeout_dir_and_chinese_writing(self) -> None:
        ...
        self.assertIn("被 taskboard 自动唤起的科研 agent", prompt)
        self.assertIn("轻度科研约定：", prompt)
        self.assertIn("写回 proposal/history 时必须说人话", prompt)
        self.assertIn("benchmark/数据集、比较对象、关键数字、变化趋势、科学含义", prompt)
        ...
        self.assertIn("正式实验默认优先用 tmux 托管", prompt)
        self.assertIn("先做对，再做快", prompt)
        self.assertIn("GPU 利用率", prompt)
    ...
    def test_build_standard_followup_prompt_uses_compact_profile(self) -> None:
        ...
        self.assertIn("同一认知线程里的本地短工作默认一次做完", prompt)
        self.assertIn("写回 proposal/history 时必须说人话", prompt)
        self.assertIn("Taskboard 操作简介：", prompt)
    ...
    def test_light_static_prompt_assets_stay_compact(self) -> None:
        ...
        self.assertIn("taskboard_runtime_prompt_zh.toml", workflow_notice)
