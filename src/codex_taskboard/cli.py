...
def followup_lock_path(config: AppConfig, followup_key: str) -> Path:
    safe_key = re.sub(r"[^A-Za-z0-9._-]+", "-", str(followup_key).strip()).strip("-") or "followup"
...
def followup_key_for(spec: dict[str, Any]) -> str:
    return hashlib.sha1(
        f"{spec.get('codex_session_id','')}::{spec.get('agent_name','')}::{spec.get('task_key', spec.get('task_id',''))}".encode("utf-8")
    ).hexdigest()[:20]
...
def build_followup_resume_spec_from_payload(followup: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "workdir": str(followup.get("workdir", "/home/Awei")),
...
def current_followup_resume_spec(config: AppConfig, followup: dict[str, Any]) -> dict[str, Any]:
    spec = build_followup_resume_spec_from_payload(followup)
    task_id = normalize_task_id(str(followup.get("task_id", "")).strip())
    if not task_id:
        return spec
    current_spec = load_task_spec(config, task_id) or {}
    current_state = load_task_state(config, task_id) or {}
    ...
    spec.update(
        {
            ...
            "codex_session_id": str(current_spec.get("codex_session_id", current_state.get("codex_session_id", spec["codex_session_id"]))),
            "proposal_path": str(current_spec.get("proposal_path", current_state.get("proposal_path", spec["proposal_path"]))),
            "proposal_source": str(current_spec.get("proposal_source", current_state.get("proposal_source", spec["proposal_source"]))),
            "proposal_owner": bool(current_spec.get("proposal_owner", current_state.get("proposal_owner", spec["proposal_owner"]))),
            "closeout_proposal_dir": str(
                current_spec.get("closeout_proposal_dir", current_state.get("closeout_proposal_dir", spec["closeout_proposal_dir"]))
            ),
            ...
            "project_history_file": str(
                current_spec.get("project_history_file", current_state.get("project_history_file", spec["project_history_file"]))
            ),
            ...
            "remote_workdir": str(current_spec.get("remote_workdir", current_state.get("remote_workdir", spec["remote_workdir"]))),
            "executor_name": str(current_spec.get("executor_name", current_state.get("executor_name", spec["executor_name"]))),
            "executor_target": str(current_spec.get("executor_target", current_state.get("executor_target", spec["executor_target"]))),
            "executor_identity_file": str(
                current_spec.get("executor_identity_file", current_state.get("executor_identity_file", spec["executor_identity_file"]))
            ),
            "executor_ssh_options": [
                str(item)
                for item in current_spec.get("executor_ssh_options", current_state.get("executor_ssh_options", spec["executor_ssh_options"]))
                if str(item).strip()
            ],
            ...
            "executor_remote_home": str(
                current_spec.get("executor_remote_home", current_state.get("executor_remote_home", spec["executor_remote_home"]))
            ),
            "executor_remote_codex_home": str(
                current_spec.get("executor_remote_codex_home", current_state.get("executor_remote_codex_home", spec["executor_remote_codex_home"]))
            ),
            "executor_remote_codex_bin": str(
                current_spec.get("executor_remote_codex_bin", current_state.get("executor_remote_codex_bin", spec["executor_remote_codex_bin"]))
            ),
            "codex_exec_mode": str(current_spec.get("codex_exec_mode", current_state.get("codex_exec_mode", spec["codex_exec_mode"]))),
            "resume_timeout_seconds": int(
                current_spec.get("resume_timeout_seconds", current_state.get("resume_timeout_seconds", spec["resume_timeout_seconds"]))
            ...
            "execution_mode": str(current_spec.get("execution_mode", current_state.get("execution_mode", spec["execution_mode"]))),
            "prompt_max_chars": int(
                current_spec.get("prompt_max_chars", current_state.get("prompt_max_chars", spec["prompt_max_chars"])) or spec["prompt_max_chars"]
            ),
            "controller_continuation_hint": (
                current_spec.get(
                    "controller_continuation_hint",
...
def merge_queued_notification_lists(existing_items: list[dict[str, Any]], incoming_items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in [*existing_items, *incoming_items]:
        if not isinstance(item, dict):
            continue
        task_id = normalize_task_id(str(item.get("task_id", "")).strip())
        if not task_id:
            passthrough.append(item)
            continue
...
def rebind_followup_to_current_task(
    ...
    if new_key != old_key and new_path.exists():
        existing_payload = read_json(new_path, {})
        ...
        atomic_write_json(new_path, merged_payload)
        sync_followup_state(
            config,
            merged_payload,
            followup_status="scheduled",
    ...
    atomic_write_json(new_path, updated_followup)
    if new_key != old_key:
        resolve_followup(config, old_key)
    sync_followup_state(
        config,
...
def followup_entity_info(config: AppConfig, task_id: str) -> tuple[bool, str]:
    ...
    if not normalized_task_id:
        return False, ""
    for payload in load_followups(config):
        if normalized_task_id in followup_task_ids(payload):
...
def schedule_followup(
    ...
    *,
    ...
    atomic_write_json(followup_path(config, followup_key), payload)
    append_followup_event_log(config, event="scheduled", reason=reason, followup=payload)
    merge_task_state(
        config,
        task_id,
        followup_status="scheduled",
...
def queue_feedback_resume(
    ...
    *,
    ...
    def update_queue() -> dict[str, Any]:
        ...
        existing_index = next(
            (index for index, item in enumerate(queued_notifications) if isinstance(item, dict) and str(item.get("task_id", "")).strip() == task_id),
            -1,
        )
        if existing_index >= 0:
        ...
        payload["executor_identity_file"] = str(spec.get("executor_identity_file", payload.get("executor_identity_file", "")))
    ...
    return run_with_followup_lock(config, followup_key, update_queue)
...
def load_followups(config: AppConfig) -> list[dict[str, Any]]:
    ...
    for path in sorted(config.followups_root.glob("*.json")):
        ...
        if isinstance(payload, dict) and payload:
            payloads.append(normalize_timestamp_fields(payload))
    return payloads
...
def followup_task_ids(followup: dict[str, Any]) -> list[str]:
    task_ids: list[str] = []
    primary_task_id = normalize_task_id(str(followup.get("task_id", "")).strip())
    if primary_task_id:
        task_ids.append(primary_task_id)
    for item in queued_notification_entries(followup):
        queued_task_id = normalize_task_id(str(item.get("task_id", "")).strip())
        if queued_task_id:
...
def active_session_followup(
    ...
    *,
    ...
    if not candidates:
        return {}
    candidates.sort(key=followup_processing_sort_key)
...
def followup_map_by_task_id(config: AppConfig) -> dict[str, dict[str, Any]]:
    ...
    for payload in load_followups(config):
        for task_id in followup_task_ids(payload):
...
def newer_task_exists(config: AppConfig, followup: dict[str, Any]) -> bool:
    ...
    source_task = load_task_state(config, source_task_id)
    comparison_floor = (
        str(followup.get("created_at", "")).strip()
        or str(source_task.get("updated_at", "")).strip()
        or str(source_task.get("submitted_at", "")).strip()
    )
    comparison_floor_ts = parse_timestamp_to_unix(comparison_floor)
    for state in iter_all_task_states(config):
        if str(state.get("task_id", "")) == source_task_id:
            continue
        submitted_at = str(state.get("submitted_at", "")).strip()
        submitted_ts = parse_timestamp_to_unix(submitted_at)
        is_newer = False
...
def defer_followup_retry(
    ...
    *,
    ...
    followup, _rebound, _skip_processing = rebind_followup_to_current_task(config, followup)
    followup_key = str(followup.get("followup_key", "")).strip()
    if not followup_key:
        return
    updated_at = utc_now()
    ...
    atomic_write_json(followup_path(config, followup_key), followup)
    append_followup_event_log(config, event="deferred", reason=reason, followup=followup)
    is_queued_feedback = str(followup.get("followup_type", "")).strip() == "queued_feedback_resume"
    for task_id in followup_task_ids(followup):
        existing_state = load_task_state(config, task_id) or {}
        merge_task_state(
            config,
            task_id,
...
def run_codex_prompt_with_continue_recovery(
    ...
    *,
    ...
    while True:
        ...
        use_remote_codex = should_use_executor_codex(spec)
        if use_remote_codex:
            command = build_remote_codex_command(
                spec or {},
                mode=current_mode,
        ...
        tracking_source_kind = normalized_feedback_source_kind or ("resume" if current_mode == "resume" and current_session_id else "")
        should_track_resume_feedback = bool(
            track_resume_feedback
            and current_mode == "resume"
            and current_session_id
        )
        if tracking_source_kind and should_track_resume_feedback:
            completed = run_tracked_feedback_subprocess(
                config,
                command,
                cwd=workdir,
            ...
            completed = run_subprocess(command, cwd=workdir, timeout=timeout_seconds)
        last_completed = completed
        combined_stdout = str(completed.stdout)
        if use_remote_codex:
            remote_message, combined_stdout = extract_remote_last_message(combined_stdout)
            if remote_message:
                ensure_dir(message_path.parent)
                message_path.write_text(remote_message, encoding="utf-8")
        combined = f"{combined_stdout}\n{completed.stderr}"
        parsed_session_id = extract_codex_session_id(combined)
        ...
        if not last_message_text and not use_remote_codex and allow_local_rollout_fallback(
            config,
            mode=current_mode,
        ...
        continue_error_kind = continue_retry_error_kind(completed.stdout, completed.stderr)
        ...
        time.sleep(min(5 * continue_attempts, 15))
...
def resume_codex_session(
    ...
    *,
    ...
    output_path = task_last_message_path(config, spec["task_id"])
    return resume_codex_session_with_prompt(
        config,
        spec,
        prompt,
        output_last_message_path=str(output_path),
        log_path=task_runner_log_path(config, spec["task_id"]),
        min_idle_seconds=min_idle_seconds,
        feedback_source_kind="task_feedback",
        feedback_source_key=str(spec.get("task_id", "")).strip(),
        feedback_task_id=str(spec.get("task_id", "")).strip(),
        feedback_task_ids=[str(spec.get("task_id", "")).strip()],
    )
...
def resume_codex_session_with_prompt(
    ...
    *,
    ...
    original_session_id = str(spec.get("codex_session_id", "")).strip()
    active_migration_entry = session_migration_entry(config, original_session_id) if original_session_id else {}
    if original_session_id and str(active_migration_entry.get("state", "")).strip() == "migrating":
        ...
        return build_deferred_resume_result(
            original_session_id=original_session_id,
    ...
    resumed_session_id = (
        session_redirect_target(config, original_session_id, include_migrating=False)
        if original_session_id
        else ""
    ) or original_session_id
    output_path = Path(output_last_message_path)
    ensure_dir(output_path.parent)
    lock_path = session_lock_path(config, resumed_session_id)
    ensure_dir(lock_path.parent)
    started_at = utc_now()
    result: dict[str, Any] = {
        "attempted": False,
    ...
    if resumed_session_id and human_guidance_mode_active(config, codex_session_id=resumed_session_id):
        retry_after = human_guidance_retry_after_seconds(config, codex_session_id=resumed_session_id)
        append_log(
            log_path,
    ...
    with lock_path.open("w", encoding="utf-8") as lock_handle:
        try:
            ...
            retry_after = default_retry_delay_seconds(min_idle_seconds)
            append_log(log_path, f"resume_deferred reason=session_locked session_id={resumed_session_id} retry_after_seconds={retry_after}")
            result.update(
                build_deferred_resume_result(
                    original_session_id=original_session_id,
                    resumed_session_id=resumed_session_id,
        ...
        if resumed_session_id:
            output_busy = session_output_busy_snapshot(config, resumed_session_id, spec=spec)
            result["latest_activity_ts"] = float(output_busy.get("latest_activity_ts", 0.0) or 0.0)
            if output_busy.get("busy", False):
                retry_after = int(output_busy.get("retry_after_seconds", DEFAULT_SESSION_OUTPUT_BUSY_RETRY_SECONDS) or DEFAULT_SESSION_OUTPUT_BUSY_RETRY_SECONDS)
                append_log(
                    log_path,
        ...
        if resumed_session_id and int(min_idle_seconds or 0) > 0:
            last_activity_ts = float(result.get("latest_activity_ts", 0.0) or latest_session_activity_ts(config, resumed_session_id, spec) or 0.0)
            result["latest_activity_ts"] = last_activity_ts
            if last_activity_ts and time.time() - last_activity_ts < int(min_idle_seconds):
                retry_after = retry_after_seconds_from_target(last_activity_ts + int(min_idle_seconds))
                append_log(
                    log_path,
        ...
        exec_result = run_codex_prompt_with_continue_recovery(
            config,
            mode="resume",
        ...
        redirect_target_after_attempt = (
            session_redirect_target(config, original_session_id, include_migrating=True)
            if original_session_id
            else ""
        )
        redirect_entry_after_attempt = session_migration_entry(config, original_session_id) if original_session_id else {}
        ...
        if primary_platform_error.get("kind") and not fallback_provider:
            retry_after = platform_error_retry_after_seconds(
        ...
        if not fallback_provider:
            result["finished_at"] = utc_now()
            return result
        ok, cloned_session_id, error = sync_thread_for_fallback(
            config,
            original_session_id=original_session_id,
        ...
        if not ok:
            ...
            result["finished_at"] = utc_now()
            return result
        fallback_output_path = task_root(config, spec["task_id"]) / f"codex-last-message-{fallback_provider}.txt"
        fallback_command = build_codex_resume_command(
            config,
        ...
        fallback_completed = run_subprocess(fallback_command, cwd=spec["workdir"], timeout=int(spec["resume_timeout_seconds"]))
        ...
        result.update(
            {
                ...
                "fallback_stdout_tail": fallback_completed.stdout[-4000:],
                "fallback_stderr_tail": fallback_completed.stderr[-4000:],
                "last_message_text": fallback_message_text or result.get("last_message_text", ""),
                "taskboard_signal": extract_taskboard_signal(
                    (fallback_message_text or str(result.get("last_message_text", ""))).strip()
                    or f"{fallback_completed.stdout}\n{fallback_completed.stderr}"
                ),
            }
        )
        append_log(
            task_runner_log_path(config, spec["task_id"]),
            f"resume_fallback returncode={fallback_completed.returncode} resumed_session_id={cloned_session_id} stdout_tail={fallback_completed.stdout[-1000:]} stderr_tail={fallback_completed.stderr[-1000:]}",
        )
        result["ok"] = fallback_completed.returncode == 0
        fallback_platform_error = classify_platform_error(fallback_completed.stdout, fallback_completed.stderr)
...
def run_codex_subagent(config: AppConfig, spec: dict[str, Any]) -> dict[str, Any]:
    ...
    output_path = subagent_last_message_path(config, task_id)
    ensure_dir(output_path.parent)
    ...
    taskboard_signal = extract_taskboard_signal(last_message or f"{completed.stdout}\n{completed.stderr}")
    return {
        "status": status,
        "returncode": completed.returncode,
        "stdout_tail": completed.stdout[-4000:],
        "stderr_tail": completed.stderr[-4000:],
        "subagent_session_id": result.get("session_id", "") or extract_codex_session_id(f"{completed.stdout}\n{completed.stderr}"),
