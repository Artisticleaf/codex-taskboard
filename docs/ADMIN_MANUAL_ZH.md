...
continuous mode 打开时，当前公开状态机只保留 `planning -> execution -> closeout` 三个阶段，以及 `EXECUTION_READY` / `WAITING_ON_ASYNC` / `CLOSEOUT_READY` / `none` 四个公开信号。管理员需要知道 closeout 转 planning 的完成条件已经变成：

- agent 已写出说人话的 closeout markdown；
- `project_history_file` 已吸收当前 proposal 的关键结论；
- handoff 已写好，并完成 proposal / history / handoff 绑定确认；
- closeout 末尾返回 `TASKBOARD_SIGNAL=none`。

如果缺少上述显式信号，taskboard 会继续保留 closeout followup，而不是把自动科研链判成已结束。
...
- 开启 continuous mode 前，先确认 agent prompt 已经理解：`WAITING_ON_ASYNC` 只表示等待 live task；`CLOSEOUT_READY` 只表示 execution 已完成“无进一步信息收益”分析；`none` 才表示当前 closeout 已完成。
